...
def save_augmented_images(data_loader: BaseDataLoader):
    ...
    for i in range(rows * cols):
        no = random.choice(list(data_loader.nissl_images.keys()))
        augmented_image = data_loader.augment_data([data_loader.nissl_images[no]])[0]
...
def show_nissl_and_avgt_images(data_loader: BaseDataLoader):
    data_loader._load_training_dataset()
    ...
    rows, cols = 3, 3
    no_list = [
        random.choice(list(data_loader.nissl_images.keys())) for _ in range(rows * cols)
    ]

    for dataset in ["nissl", "avgt"]:

        if dataset == "nissl":
            images = [data_loader.nissl_images[no] for no in no_list]
        elif dataset == "avgt":
            images = [data_loader.avgt_images[no] for no in no_list]
        else:
...
def save_top_n_predictions(
    ...
    images = [
        ...
        (data_loader.avgt_images[brain_slice_path], brain_slice_path),
    ]
    images += [
        (data_loader.avgt_images[atlas_path], atlas_path)
    ...
    for i in range(cols * rows):
        ...
        position, degree = images[i][1].split("_")
        if i == 0:
            ax.title.set_text(f"Brain slice: {position} {degree}{degree_sign}")
        elif i == 1:
            ax.title.set_text(f"Actual atlas plate: {position} {degree}{degree_sign}")
        else:
            ax.title.set_text(
                f"Predicted plate: {position} {degree}{degree_sign}",
            )
        ax.title.set_size(9)
...
if __name__ == "__main__":
    ...
    # show_nissl_and_avgt_images(data_loader)
